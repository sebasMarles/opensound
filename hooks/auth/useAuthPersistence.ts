...
/**
 * Extrae el token de un valor almacenado en AsyncStorage
 * Maneja diferentes formatos: string directo o objeto JSON con propiedad token
 */
...
/**
 * Lee el usuario almacenado en AsyncStorage
 */
...
/**
 * Hook para manejar la persistencia de la sesión de autenticación
 * Separado de useAuthSession para seguir el principio de responsabilidad única
 */
export function useAuthPersistence() {
  ...
  const persistSession = useCallback(async (user: AuthUser, token: string) => {
    await Promise.all([
      AsyncStorage.setItem(AUTH_TOKEN_STORAGE_KEY, JSON.stringify({ token })),
      AsyncStorage.setItem(AUTH_USER_STORAGE_KEY, JSON.stringify(user)),
    ])
  }, [])

  const clearSessionStorage = useCallback(async () => {
    await Promise.all([AsyncStorage.removeItem(AUTH_TOKEN_STORAGE_KEY), AsyncStorage.removeItem(AUTH_USER_STORAGE_KEY)])
  }, [])
