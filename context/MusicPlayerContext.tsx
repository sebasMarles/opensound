...
interface MusicPlayerContextType {
  ...
  playPlaylist: (songs: any[], startIndex?: number) => Promise<void>
}
...
export const MusicPlayerProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const attachStatusListener = useCallback((player: AudioPlayer) => {
    statusSubscriptionRef.current?.remove?.()
    statusSubscriptionRef.current = player.addListener("playbackStatusUpdate", (status: AudioStatus) => {
  ...
  const loadAndPlay = useCallback(
    async (song: CurrentSong) => {
      ...
      // Pausar y limpiar el player actual
      if (player.playing) {
        player.pause()
      }
  ...
  const setCurrentSong: React.Dispatch<React.SetStateAction<CurrentSong | null>> = (value) => {
  ...
  const playPlaylist = useCallback(
    async (songs: any[], startIndex = 0) => {
      ...
      const mapped: CurrentSong[] = songs
        .map((song) => ({
          id: song.jamendoId || song.id,
          title: song.name || song.title || "Sin título",
          artist: song.artist_name || song.artist || "Artista desconocido",
          image: song.album_image || song.image || "https://picsum.photos/200",
          audio: song.audio,
        }))
        .filter((s) => !!s.audio)
      ...
      setQueue(mapped)
      const validIndex = Math.max(0, Math.min(startIndex, mapped.length - 1))
      setCurrentIndex(validIndex)

      const song = mapped[validIndex]
      _setCurrentSong(song)
      await loadAndPlay(song)
    },
    [loadAndPlay],
